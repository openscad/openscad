...
Compile OpenSCAD in debug mode and always run OpenSCAD from the cmd-line using the argument --debug=FileModule,SourceFileCache.
...
o Turn off Design->Automatic Reload and Preview
o Open use.scad
o Preview twice (F5) - check that module reference is the same and that ModuleCache only says "Compiling library: /path/to/used.scad" once.
o touch used.scad
o Preview (F5) - check that ModuleCache says "Recompiling cached library: /path/to/used.scad".
...
o Preview (F5) - verify that you get a sphere and a cube of approximately the same size
o Edit multipleB.scad:
  - cube(1.5*F(), center=true);
  + cube(2.5*F(), center=true);
o Reload and Preview (F4) - verify that the cube got larger
o Edit multiplecommon.scad:
  - function F() = 20;
  + function F() = 10;
o Reload and Preview (F4) - verify that the sphere and the cube both got smaller

Test7: Dependency tracking of file included from module
------

o Open includefrommodule.scad
o Preview (F5) - Verify that you get a circular disc
o Edit radius.scad: Change RADIUS
o Preview (F5) - Verify that the disc changed size
...
o Reload and Preview (F4) - verify that the sphere appeared
o rm missing.scad
o Reload and Preview (F4) - verify that the sphere is still there
o echo "module missing() { sphere(20); }" >  missing.scad
o Reload and Preview (F4) - verify that the sphere increased in size
...
o Turn on Automatic Reload and Preview
