...
def compare_png(resultfilename):
    ...
    args = [expectedfilename, resultfilename, "-background", "magenta", "-alpha", "Remove", "-compose", "difference", "-composite", "-black-threshold", "10%", "-white-threshold", "10%", "-morphology", "Erode", "Square", "-format", "%[fx:w*h*mean]", "info:"]
    ...
    if options.comparator == 'old':
      args = [expectedfilename, resultfilename, "-background", "magenta", "-alpha", "Remove", "-compose", "difference", "-composite", "-threshold", "10%", "-gaussian-blur","3x65535", "-threshold", "99.99%", "-format", "%[fx:w*h*mean]", "info:"]

    if options.comparator == 'ncc':
      # for systems where imagemagick crashes when using the above comparators
      args = [expectedfilename, resultfilename, "-background", "magenta", "-alpha", "Remove", "-compose", "difference", "-metric", "NCC", "tmp.png"]
