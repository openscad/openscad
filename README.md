...
Once built, run regression tests from the `build` directory with e.g. `ctest -L Default` (test groups are selected with `-L`; on multi-config generators like Visual Studio, also pass `-C Release` or `-C Debug` for the build type).
