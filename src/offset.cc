...
void register_builtin_offset()
{
	...
	offset.push_back("offset(r = number)");
	offset.push_back("offset(delta = number)");
	offset.push_back("offset(r = number, chamfer = boolean)");
	Builtins::init("offset", new OffsetModule(), offset);
}
