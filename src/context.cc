...
const Context::Expressions Context::getExpressions(const AssignmentList &args, const EvalContext *evalctx)
{
	...
	for(const auto &arg : args) {
		expressions[arg.first] = arg.second.get(); // NOTE: this can assign NULL pointers!
	}
	...
	if (evalctx) {
		...
		for (size_t i=0; i<evalctx->numArgs(); i++) {
			...
			const Expression *expr = evalctx->getArgs()[i].second.get();
			if (name.empty()) {
				if (posarg < args.size()) {
					const Assignment assignment = args[posarg++];
					expressions[assignment.first] = expr;
				}
			} else {
