...
std::shared_ptr<PolySet> ManifoldGeometry::toPolySet() const
{
  ...
  // Faces with no color of their own are tagged rather than colored. Resolving them here would
  // bake the color scheme that happened to be active into geometry that is then cached and
  // redrawn under whatever scheme the user picks next; the renderers and the exporters resolve
  // the tags instead, each against the scheme in force when they run.
  auto getColorIndex = [&](uint32_t originalID) -> int32_t {
    if (subtractedIDs_.find(originalID) != subtractedIDs_.end()) {
      return color_index_t::kCutout;
    }
