...
{
  IndexedTriangleMesh im;
  PolySetUtils::tessellate_faces(ps, im);

  auto numfaces = im.triangles.size();
  const auto &vertices = im.vertices;
  ...
  std::transform(im.triangles.begin(), im.triangles.end(), mesh.triVerts.begin(),
    [&](const auto &t) -> glm::ivec3 {
      assert(t[0] >= 0 && t[0] < vertexCount &&
            t[1] >= 0 && t[1] < vertexCount &&
            t[2] >= 0 && t[2] < vertexCount);
