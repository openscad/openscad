...
{
  size_t buffer_size = 0;
  for (const auto& poly : polyset.polygons_ind) {
    if (poly.size() == 3) {
      buffer_size++;
    } else if (poly.size() == 4) {
      buffer_size += 2;
                                 ...
                                 csgmode_e csgmode, const Transform3d& m, const Color4f& color) const
{
  ...
  if (ps.getDimension() == 2) {
    ...
    for (const auto& poly : ps.polygons_ind) {
      if (poly.size() == 3) {
        Vector3d p0 = uniqueMultiply(vert_mult_map, mult_verts, ps.points[poly.at(0)], m);
        Vector3d p1 = uniqueMultiply(vert_mult_map, mult_verts, ps.points[poly.at(1)], m);
        Vector3d p2 = uniqueMultiply(vert_mult_map, mult_verts, ps.points[poly.at(2)], m);
        ...
        Vector3d p1 = uniqueMultiply(vert_mult_map, mult_verts, ps.points[poly.at(1)], m);
        Vector3d p2 = uniqueMultiply(vert_mult_map, mult_verts, ps.points[poly.at(2)], m);
        Vector3d p3 = uniqueMultiply(vert_mult_map, mult_verts, ps.points[poly.at(3)], m);
        ...
        for (size_t i = 1; i <= poly.size(); i++) {
          ...
          Vector3d p1 = uniqueMultiply(vert_mult_map, mult_verts, ps.points[poly.at(i % poly.size())], m);
          Vector3d p2 = uniqueMultiply(vert_mult_map, mult_verts, ps.points[poly.at(i - 1)], m);
  ...
  if (ps.getDimension() == 2) {
    ...
    for (const auto& polygon : ps.polygons_ind) {
      ...
      for (const auto& vertex : polygon) {
        Vector3d p = uniqueMultiply(vert_mult_map, mult_verts, ps.points[vertex], m);
  ...
  if (ps.getDimension() == 2) {
    ...
    if (csgmode == Renderer::CSGMODE_NONE) {
      ...
      for (double z : { -zbase / 2, zbase / 2}) {
        for (const auto& poly : ps.polygons_ind) {
          if (poly.size() == 3) {
            Vector3d p0 = ps.points[poly.at(0)]; p0[2] += z;
            Vector3d p1 = ps.points[poly.at(1)]; p1[2] += z;
            Vector3d p2 = ps.points[poly.at(2)]; p2[2] += z;
            ...
            Vector3d p1 = ps.points[poly.at(1)]; p1[2] += z;
            Vector3d p2 = ps.points[poly.at(2)]; p2[2] += z;
            Vector3d p3 = ps.points[poly.at(3)]; p3[2] += z;
            ...
            for (size_t i = 1; i <= poly.size(); i++) {
              ...
              Vector3d p1 = ps.points[poly.at(i % poly.size())]; p1[2] += z;
              Vector3d p2 = ps.points[poly.at(i - 1)]; p2[2] += z;
      ...
      if (ps.getPolygon().outlines().size() > 0) {
        ...
        for (const auto& poly : ps.polygons_ind) {
          for (size_t i = 1; i <= poly.size(); i++) {
            Vector3d p1 = ps.points[poly.at(i - 1)]; p1[2] -= zbase / 2;
            Vector3d p2 = ps.points[poly.at(i - 1)]; p2[2] += zbase / 2;
            Vector3d p3 = ps.points[poly.at(i % poly.size())]; p3[2] -= zbase / 2;
            Vector3d p4 = ps.points[poly.at(i % poly.size())]; p4[2] += zbase / 2;
