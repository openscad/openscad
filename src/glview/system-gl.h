...
#ifdef USE_GLAD
#ifdef _WIN32
#define NORESOURCE  // To avoid picking up DIFFERENCE from winuser.h, conflicting with
...
//
// This example will print an error if glClear() fails if --debug is specified, but yields just glClear
// in release mode. GL_DEBUG_CHECKD(glClear());
#ifdef DEBUG
