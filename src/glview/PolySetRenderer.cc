...
// This renderer is using in Manifold and FastCsg mode (F6 with Manifold or FastCsg as geometry engine)
...
void PolySetRenderer::addGeometry(const std::shared_ptr<const Geometry>& geom)
{
  if (const auto geomlist = std::dynamic_pointer_cast<const GeometryList>(geom)) {
    ...
    // Note: It's rare, but possible for Nef polyhedrons to exist among geometries in Manifold or FastCsg mode.
...
void PolySetRenderer::createVertexStates()
{
  ...
  VertexArray vertex_array(std::make_unique<VertexStateFactory>(), vertex_states_, vertices_vbo_, elements_vbo_);
...
void PolySetRenderer::prepare(bool /*showfaces*/, bool /*showedges*/, const shaderinfo_t * /*shaderinfo*/)
{
  PRINTD("prepare()");
  if (!vertex_states_.size()) createVertexStates();
  PRINTD("prepare() end");
}

void PolySetRenderer::draw(bool showfaces, bool showedges, const shaderinfo_t * /*shaderinfo*/) const
{
  PRINTD("draw()");
