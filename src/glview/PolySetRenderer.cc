...
void PolySetRenderer::createVertexStates()
{
  ...
  if (!polysets_.empty() && !polygons_.empty()) {
    LOG("Warning: PolySetRenderer has both 2D and 3D geometry. Only 3D geometry will be rendered.");
  }
  if (!polysets_.empty()) {
    ...
    for (const auto& polyset : this->polysets_) {
      ...
      add_color(vbo_builder, color);
      this->create_surface(*polyset, vbo_builder, RendererUtils::CSGMODE_NORMAL,
...
//    vbo_builder.addSurfaceData();
    ...
    for (const auto &[polygon, polyset] : this->polygons_) {
      ...
      this->create_polygons(*polyset, vbo_builder, Transform3d::Identity(),
                            color);
