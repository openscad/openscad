...
void Renderer::render_surface(const PolySet& ps, csgmode_e csgmode, const Transform3d& m, const shaderinfo_t *shaderinfo) const
{
  ...
  if (ps.getDimension() == 2) {
    ...
    for (double z : {-zbase / 2, zbase / 2}) {
      for (const auto& poly : ps.polygons_ind) {
        if (poly.size() == 3) {
          if (z < 0) {
            gl_draw_triangle(shaderinfo, ps.points[poly.at(0)], ps.points[poly.at(2)], ps.points[poly.at(1)], true, true, true, z, mirrored);
          } else {
            gl_draw_triangle(shaderinfo, ps.points[poly.at(0)], ps.points[poly.at(1)], ps.points[poly.at(2)], true, true, true, z, mirrored);
          }
        } else if (poly.size() == 4) {
          if (z < 0) {
            gl_draw_triangle(shaderinfo, ps.points[poly.at(0)], ps.points[poly.at(3)], ps.points[poly.at(1)], false, true, true, z, mirrored);
            gl_draw_triangle(shaderinfo, ps.points[poly.at(2)], ps.points[poly.at(1)], ps.points[poly.at(3)], false, true, true, z, mirrored);
          } else {
            gl_draw_triangle(shaderinfo, ps.points[poly.at(0)], ps.points[poly.at(1)], ps.points[poly.at(3)], false, true, true, z, mirrored);
            gl_draw_triangle(shaderinfo, ps.points[poly.at(2)], ps.points[poly.at(3)], ps.points[poly.at(1)], false, true, true, z, mirrored);
          }
        } else {
          Vector3d center = Vector3d::Zero();
          for (const auto& point : poly) {
            center[0] += ps.points[point][0];
            center[1] += ps.points[point][1];
          }
          center /= poly.size();
          for (size_t j = 1; j <= poly.size(); ++j) {
            if (z < 0) {
              gl_draw_triangle(shaderinfo, center, ps.points[poly.at(j % poly.size())], ps.points[poly.at(j - 1)],
                               true, false, false, z, mirrored);
            } else {
              gl_draw_triangle(shaderinfo, center, ps.points[poly.at(j - 1)], ps.points[poly.at(j % poly.size())],
                               true, false, false, z, mirrored);
            }
          }
        }
    ...
    if (ps.getPolygon().outlines().size() > 0) {
      ...
      const std::vector<IndexedFace> *borders_p = &ps.polygons_ind;
      for (const auto& poly : *borders_p) {
        for (size_t j = 1; j <= poly.size(); ++j) {
          Vector3d p1 = ps.points[poly.at(j - 1)], p2 = ps.points[poly.at(j - 1)];
          Vector3d p3 = ps.points[poly.at(j % poly.size())], p4 = ps.points[poly.at(j % poly.size())];
    ...
    for (const auto& poly : ps.polygons_ind) {
      glBegin(GL_TRIANGLES);
      if (poly.size() == 3) {
        gl_draw_triangle(shaderinfo, ps.points[poly.at(0)], ps.points[poly.at(1)], ps.points[poly.at(2)], true, true, true, 0, mirrored);
      } else if (poly.size() == 4) {
        gl_draw_triangle(shaderinfo, ps.points[poly.at(0)], ps.points[poly.at(1)], ps.points[poly.at(3)], false, true, true, 0, mirrored);
        gl_draw_triangle(shaderinfo, ps.points[poly.at(2)], ps.points[poly.at(3)], ps.points[poly.at(1)], false, true, true, 0, mirrored);
      } else {
        Vector3d center = Vector3d::Zero();
        for (const auto& point : poly) {
          center += ps.points[point];
        }
        center /= poly.size();
        for (size_t j = 1; j <= poly.size(); ++j) {
          gl_draw_triangle(shaderinfo, center, ps.points[poly.at(j - 1)], ps.points[poly.at(j % poly.size())], true, false, false, 0, mirrored);
        }
      }
      glEnd();
    }
...
void Renderer::render_edges(const PolySet& ps, csgmode_e csgmode) const
{
  ...
  if (ps.getDimension() == 2) {
    ...
    for (const auto& polygon : ps.polygons_ind) {
      const IndexedFace *poly = &polygon;
      glBegin(GL_LINE_LOOP);
      for (const auto& ind : *poly) {
	Vector3d p=ps.points[ind];
