...
public:
  ...
  int index(Vertex_iterator v) const { return    VI [v]; }
	...
	template <typename NT> bool    read_edge (iarchive &ia, Halfedge_handle);
  template <typename NT> bool   read_facet (iarchive &ia, Halffacet_handle);
  bool                         read_volume (iarchive &ia, Volume_handle);
  template <typename NT> bool read_svertex (iarchive &ia, SVertex_handle);
  template <typename NT> bool   read_sedge (iarchive &ia, SHalfedge_handle);
  template <typename NT> bool   read_sloop (iarchive &ia, SHalfloop_handle);
...
{ 
	...
	if(reduce) {
	  Geometry_io<typename Standard_kernel::Kernel_tag, Standard_kernel>::
	    print_point(oa, Infi_box::standard_point(v->point()));
	}
	else
...
{ 
  ...
  if ( D.is_isolated(e) ) oa &true &index(e->incident_sface());
  else oa &false &index(e->out_sedge());
  if(reduce) {
    Standard_point sp = Infi_box::standard_point(e->point());
    Geometry_io<typename Standard_kernel::Kernel_tag, Standard_kernel>::
      print_vector(oa, sp-CGAL::ORIGIN);
  }
  else
    Geometry_io<typename Kernel::Kernel_tag, Kernel>::
      print_vector(oa, e->vector());
...
bool SNC_io_parser<EW>::read_edge(iarchive &ia, Halfedge_handle eh)
{
	...
	#else
	  eh->point() =
	    Geometry_io<typename K::Kernel_tag, Kernel>::template read_point<Kernel,K>(ia);
	#endif
	  ia &eh->mark();
...
{ 
  ...
  CGAL_forall_facet_cycles_of(it,f)
    if ( it.is_shalfedge() ) oa &index(SHalfedge_handle(it));
  CGAL_forall_facet_cycles_of(it,f)
    if ( it.is_shalfloop() ) oa &index(SHalfloop_handle(it));
  oa &index(f->incident_volume());
  if(reduce) {
    Geometry_io<typename Standard_kernel::Kernel_tag, Standard_kernel>::
      print_plane(oa, Infi_box::standard_plane(f->plane()));
  }
  else
...
bool SNC_io_parser<EW>::read_facet(iarchive &ia, Halffacet_handle fh)
{
    ...
    for(auto i = 0; i < n_shalfloop; ++i){
	  ...
	  fh->plane() =
	    ...
	    template read_plane<Kernel, K>(ia);
	#endif
	  ia &fh->mark();
...
void SNC_io_parser<EW>::print_volume(oarchive &oa, Volume_handle c) const
{
  ...
  CGAL_forall_shells_of(it,c)
    if(!reduce || Infi_box::is_standard(SFace_handle(it)->center_vertex()->point())) ++count;
  oa &count;
  CGAL_forall_shells_of(it,c)
...
print_sedge(oarchive &oa, SHalfedge_handle e) const {
//index { twin, sprev, snext, source, sface, prev, next, facet | circle } mark
  ...
  if(reduce) {
    Geometry_io<typename Standard_kernel::Kernel_tag, Standard_kernel>::
      print_plane(oa, Infi_box::standard_plane(e->circle()));
  }
  else
    Geometry_io<typename Kernel::Kernel_tag, Kernel>::
      print_plane(oa, (Plane_3) e->circle());

  oa &e->mark();
#ifdef CGAL_NEF_OUTPUT_INDEXES
  oa &e->get_forward_index()
...
template <typename EW>
void SNC_io_parser<EW>::
print_sloop(oarchive &oa, SHalfloop_handle l) const
{
  oa
  ...
  if(reduce) {
    Geometry_io<typename Standard_kernel::Kernel_tag, Standard_kernel>::
      print_plane(oa, Infi_box::standard_plane(l->circle()));
  }
  else
    Geometry_io<typename Kernel::Kernel_tag, Kernel>::
      print_plane(oa, (Plane_3) l->circle());
...
template <typename EW>
void SNC_io_parser<EW>::
print_sface(oarchive &oa, SFace_handle f) const
{
  ...
  CGAL_forall_sface_cycles_of(it,f)
    if ( it.is_shalfedge() ) oa &index(SHalfedge_handle(it));
  CGAL_forall_sface_cycles_of(it,f)
    if ( it.is_svertex() ) oa &index(SVertex_handle(it));
  CGAL_forall_sface_cycles_of(it,f)
    if ( it.is_shalfloop() ) oa &index(SHalfloop_handle(it));
