...
const Geometry *SphereNode::createGeometry() const
{
	...
	if (this->r > 0 && !std::isinf(this->r)) {
		struct ring_s {
			point2d *points;
			double z;
		};
		
		int fragments = Calc::get_fragments_from_r(this->r, this->fn, this->fs, this->fa);
...
//			double phi = (M_PI * (i + offset)) / (fragments/2);
			...
			double r = this->r * sin(phi);
			ring[i].z = this->r * cos(phi);
...
const Geometry *CircleNode::createGeometry() const
{
	...
	if (this->r > 0 && !std::isinf(this->r))	{
		...
		for (int i=0; i < fragments; i++) {
			double phi = (M_PI*2*i) / fragments;
			o.vertices[i] = Vector2d(this->r*cos(phi), this->r*sin(phi));
		}
		p->addOutline(o);
	}
	p->setSanitized(true);
...
std::string CubeNode::toString() const
{
	...
	stream << this->name() << "(size = [" << this->x << ", " << this->y << ", " << this->z << "], "
...
{
	std::stringstream stream;
	stream << this->name() << "($fn = " << this->fn << ", $fa = " << this->fa
				 << ", $fs = " << this->fs << ", r = " << this->r << ")";
	return stream.str();
}
...
{
	std::stringstream stream;
	stream << this->name() << "(points = " << *this->points
				 << ", faces = " << *this->faces
...
{
	std::stringstream stream;
	stream << this->name() << "(size = [" << this->x << ", " << this->y << "], "
...
{
	std::stringstream stream;
	stream << this->name() << "(points = " << *this->points << ", paths = " << *this->paths
				 << ", convexity = " << this->convexity << ")";
	return stream.str();
}
