...
hb_direction_t FreetypeRenderer::Params::get_direction(const hb_script_t script) const
{
	hb_direction_t param_direction = hb_direction_from_string(direction.c_str(), -1);
...
hb_script_t FreetypeRenderer::Params::get_script(hb_glyph_info_t *glyph_info, unsigned int glyph_count) const
{
	hb_script_t param_script = hb_script_from_string(script.c_str(), -1);
	...
	PRINTDB("Detected script '%c%c%c%c' for %s", SCRIPT_UNTAG(script) % text.c_str());
	return script;
}
...
void FreetypeRenderer::Params::detect_properties()
{
	...
	hb_script_t script = get_script(glyph_info, glyph_count);
	...
	hb_direction_t direction = get_direction(script);
	set_direction(hb_direction_to_string(direction));
