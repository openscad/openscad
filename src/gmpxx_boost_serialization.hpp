...
inline void serialize(Archive &ar, mpz_class &z, const unsigned int version)
{
...
template <typename Archive>
inline void serialize(Archive &ar, __mpz_struct &z, const unsigned int version)
{
	split_free(ar, z, version);
}

template <typename Archive>
inline void load(Archive &ar, __mpz_struct &z, const unsigned int)
{
	// do the POD part
...
template <typename Archive>
inline void save(Archive &ar, const __mpz_struct &z, const unsigned int)
{
...
template <typename Archive>
inline void serialize(Archive &ar, mpq_class &q, const unsigned int version)
{
	ar &*(q.get_mpq_t()); // `.mp` is the sole data member of mp*_class types
};

template <typename Archive>
inline void serialize(Archive &ar, __mpq_struct &q, const unsigned int version)
{
	ar &q._mp_num &q._mp_den;
};

template <typename Archive>
inline void serialize(Archive &ar, mpf_class &f, const unsigned int version)
{
...
template <typename Archive>
inline void load(Archive &ar, __mpf_struct &f, const unsigned int version)
{
	...
	// free old memory
	if (original_limb_no != 0) (*free_func_ptr)(f._mp_d, original_limb_no);
	// make room for arriving limbs
...
template <typename Archive>
inline void save(Archive &ar, __mpf_struct &f, const unsigned int version)
{
	ar &f._mp_prec &f._mp_size &f._mp_exp;
	// now serialize the limbs array
	mp_limb_t *limbp = f._mp_d;
