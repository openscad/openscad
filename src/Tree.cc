...
*/
const std::string &Tree::getString(const AbstractNode &node, bool use_indent) const
{
	assert(this->root_node);
	if (!this->nodecache.contains(node)) {
		this->nodecache.clear();
		this->nodeidcache.clear();
		NodeDumper dumper(this->nodecache, use_indent, false);
...
const std::string &Tree::getIdString(const AbstractNode &node) const
{
	...
	if (!this->nodeidcache.contains(node)) {
		const std::string &nodestr = getString(node, false);
