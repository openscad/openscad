...
static constexpr int MAX_LOG_SIZE = 1024 * 1024;
...
void HidApiInputDriver::generic_loadDescriptor()
{
  ...
  for (size_t i = 0; i < descriptorSize; i += len) {
    ...
    int bDataSize;
    int tag;
    // HIDAPI_LOGP("%04zx: bSize %d bType %d bTag 0x%x", i % bSize % bType % bTag);
    if (bTag == 0x0f) {
    ...
    // on Logical Maximum.
    if (bSize < 4 && (dataVal & (1 << (bSize*8 - 1)))) {
      sDataVal = dataVal | (-1 << bSize*8);
    } else {
      sDataVal = dataVal;
    }
    ...
    switch (bType) {
      case HID_TYPE_MAIN:
        ...
        switch (tag) {
          case HID_MAIN_INPUT:
            HIDAPI_LOGP("%s Input 0x%02x", msg % dataVal);
            enum {
            ...
            if (dataVal & HID_INPUT_CONSTANT) {
              offsetBits += global.report.size * global.report.count;
              break;
            }
            if (!(dataVal & HID_INPUT_VARIABLE)) {
              // No support for ARRAY yet.
              offsetBits += global.report.size * global.report.count;
              break;
            }
            if (dataVal & HID_INPUT_RELATIVE) {
              // No support for RELATIVE yet.
              offsetBits += global.report.size * global.report.count;
              break;
            }
          ...
          case HID_MAIN_OUTPUT:
            HIDAPI_LOGP("%s Output 0x%02x", msg % dataVal);
            break;
          case HID_MAIN_FEATURE:
            HIDAPI_LOGP("%s Feature 0x%02x", msg % dataVal);
            break;
          case HID_MAIN_COLLECTION:
            HIDAPI_LOGP("%s Collection 0x%02x", msg % dataVal);
            break;
      ...
      case HID_TYPE_GLOBAL:
        ...
        switch (tag) {
          case HID_GLOBAL_USAGE_PAGE:
            HIDAPI_LOGP("%s Usage page 0x%02x", msg % dataVal);
            global.usagePage = dataVal;
            break;
          case HID_GLOBAL_LMIN:
            HIDAPI_LOGP("%s Logical minimum 0x%02x", msg % dataVal);
            global.logical.minimum = sDataVal;
            break;
          case HID_GLOBAL_LMAX:
            // Work around a bug in some descriptors (notably Logitech Gamepad F310) where the
            // maximum is erroneously treated as an unsigned value.  (It's a single-byte FF,
            // which should be treated as -1 but the device's intent is +255.)  When we actually
            // go to use it, if the maximum is less than the minimum, we assume that it should
            // have been treated as unsigned.
            HIDAPI_LOGP("%s Logical maximum 0x%02x", msg % dataVal);
            global.logical.smaximum = sDataVal;
            global.logical.umaximum = dataVal;
            break;
          case HID_GLOBAL_PMIN:
            HIDAPI_LOGP("%s Physical minimum 0x%02x", msg % dataVal);
            global.physical.minimum = sDataVal;
            break;
          case HID_GLOBAL_PMAX:
            HIDAPI_LOGP("%s Physical maximum 0x%02x", msg % dataVal);
            global.physical.maximum = sDataVal;
            break;
          case HID_GLOBAL_UNIT_EXP:
            HIDAPI_LOGP("%s Unit exponent 0x%02x", msg % dataVal);
            // No support for units yet.
            break;
          case HID_GLOBAL_UNIT:
            HIDAPI_LOGP("%s Unit 0x%02x", msg % dataVal);
            // No support for units yet.
            break;
          case HID_GLOBAL_REPORT_SIZE:
            HIDAPI_LOGP("%s Report size 0x%02x", msg % dataVal);
            global.report.size = dataVal;
            break;
          case HID_GLOBAL_REPORT_ID:
            HIDAPI_LOGP("%s Report ID 0x%02x", msg % dataVal);
            global.id = dataVal;
            break;
          case HID_GLOBAL_REPORT_COUNT:
            HIDAPI_LOGP("%s Report count %d", msg % dataVal);
            global.report.count = dataVal;
            break;
      ...
      case HID_TYPE_LOCAL:
        ...
        switch (tag) {
          case HID_LOCAL_USAGE:
            HIDAPI_LOGP("%s Usage 0x%02x, %02x", msg % global.usagePage % dataVal);
            local.usages.push_back(dataVal);
            break;
          case HID_LOCAL_USAGE_MIN:
            HIDAPI_LOGP("%s Usage min 0x%02x, %02x", msg % global.usagePage % dataVal);
            local.usageMinimum = dataVal;
            break;
          case HID_LOCAL_USAGE_MAX:
            HIDAPI_LOGP("%s Usage max 0x%02x, %02x", msg % global.usagePage % dataVal);
            local.usageMaximum = dataVal;
            break;
          case HID_LOCAL_D_INDEX:
            HIDAPI_LOGP("%s Designator index 0x%02x", msg % dataVal);
            // Not supported
            break;
          case HID_LOCAL_D_MIN:
            HIDAPI_LOGP("%s Designator min 0x%02x", msg % dataVal);
            // Not supported
            break;
          case HID_LOCAL_D_MAX:
            HIDAPI_LOGP("%s Designator max 0x%02x", msg % dataVal);
            // Not supported
            break;
          case HID_LOCAL_S_INDEX:
            HIDAPI_LOGP("%s String index %02x", msg % dataVal);
            // Not supported
            break;
          case HID_LOCAL_S_MIN:
            HIDAPI_LOGP("%s String min %02x", msg % dataVal);
            // Not supported
            break;
          case HID_LOCAL_S_MAX:
            HIDAPI_LOGP("%s String max %02x", msg % dataVal);
            // Not supported
            break;
          case HID_LOCAL_DELIMITER:
            HIDAPI_LOGP("%s Delimiter %d", msg % dataVal);
            // Not supported
            break;
          default:
            HIDAPI_LOGP("%s tag 0x%02x", msg % tag);
...
{
  for (std::shared_ptr<UsageInfo> u : items) {
