...
{
  const auto& service = S::printServiceName.value();
  ...
  if (printService != print_service_t::NONE) {
    ...
    this->selectedServiceName = QString::fromStdString(service);

    std::string fileFormatString;
    switch (printService) {
    case print_service_t::PRINT_SERVICE:
      fileFormatString = S::printServiceFileFormat.value();
      break;
    case print_service_t::OCTOPRINT:
      fileFormatString = S::octoPrintFileFormat.value();
      break;
    case print_service_t::LOCAL_APPLICATION:
      fileFormatString = S::localAppFileFormat.value();
      break;
    default:
      break;
    }
    if (!fileformat::fromIdentifier(fileFormatString,
