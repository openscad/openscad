...
FontCache::FontCache()
{
	...
	if (!this->config) {
		LOG("",-1,"Can't initialize fontconfig library, text() objects will not be rendered",message_group::Font_Warning);
		return;
	}
	...
	if (error) {
		LOG("",-1,"Can't initialize freetype library, text() objects will not be rendered",message_group::Font_Warning);
		return;
	}
...
void FontCache::register_font_file(const std::string &path)
{
	if (!FcConfigAppFontAddFile(this->config, reinterpret_cast<const FcChar8 *> (path.c_str()))) {
		LOG("",-1,getFormatted("Can't register font '%1$s'",path),message_group::None);
	}
}
...
void FontCache::add_font_dir(const std::string &path)
{
	...
	if (!FcConfigAppFontAddDir(this->config, reinterpret_cast<const FcChar8 *> (path.c_str()))) {
		LOG("",-1,getFormatted("Can't register font directory '%1$s'",path),message_group::None);
	}
}
...
FT_Face FontCache::find_face_fontconfig(const std::string &font) const
{
	...
	if (FT_Select_Charmap(face, ft_encoding_unicode) == 0) {
		...
		if (!charmap_set)
			LOG("",-1,getFormatted("Could not select a char map for font %1$s/%2$s'",face->family_name,face->style_name),message_group::Font_Warning);
	}
