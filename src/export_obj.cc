...
static void append_obj(const std::vector<Vector3d> &vecs, const std::vector<IndexedTriangle> &tris, std::ostream &output)
{
 ...
 */
static void append_obj(const CGAL_Nef_polyhedron &root_N, std::ostream &output)
{
	...
	if (usePolySet) {
		...
		if (err) { PRINT("ERROR: Nef->PolySet failed"); }
			...
			append_obj(r.first, r.second, output);
		}
	}
	else {
...
static void append_obj(const shared_ptr<const Geometry> &geom, std::ostream &output)
{
...
void export_obj(const shared_ptr<const Geometry> &geom, std::ostream &output)
{
	...
	output << "# Generated by OpenSCAD " << QUOTED(OPENSCAD_VERSION) << "\r\n";

	append_obj(geom, output);
