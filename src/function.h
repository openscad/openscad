...
public:
	...
	virtual ValuePtr evaluate(const class Context *ctx, const class EvalContext *evalctx, const Location &loc) const = 0;
};

class BuiltinFunction : public AbstractFunction
{
public:
	typedef ValuePtr (*eval_func_t)(const Context *ctx, const EvalContext *evalctx, const Location &loc);
