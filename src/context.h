...
public:
	...
	void setVariables(const AssignmentList &args, const class EvalContext *evalctx = nullptr, bool silentOverwrite=true, const Location &loc=Location::NONE);

	void set_variable(const std::string &name, const ValuePtr &value, bool silentOverwrite=true, const Location &loc=Location::NONE);
	void set_variable(const std::string &name, const Value &value, bool silentOverwrite=true, const Location &loc=Location::NONE);
	void set_constant(const std::string &name, const ValuePtr &value);
	void set_constant(const std::string &name, const Value &value);

	void apply_variables(const Context &other,  bool silentOverwrite=true, const Location &loc=Location::NONE);
