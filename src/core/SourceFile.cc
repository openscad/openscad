...
SourceFile::SourceFile(std::string path, std::string filename, std::shared_ptr<LocalScope> new_scope)
  : ASTNode(Location::NONE),
    path(std::move(path)),
    filename(std::move(filename)),
    scope(new_scope)
  ...
  std::shared_ptr<const FileContext> *resulting_file_context) const
{
  ...
  try {
    ContextHandle<FileContext> file_context{Context::create<FileContext>(parent, this)};
...
const std::vector<std::string>& SourceFile::getNamespaceUsedLibrariesReverseOrdered(const std::string& name) const
{
