...
boost::optional<CallableFunction> BuiltinContext::lookup_function_as_namespace(const std::string& name) const
{
  return lookup_local_function(name, Location::NONE);
}

boost::optional<InstantiableModule> BuiltinContext::lookup_module_as_namespace(const std::string& name) const
{
