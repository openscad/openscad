...
template boost::optional<const Value&> EvaluationSession::lookup_namespace<const Value&>(const std::string&, const std::string&) const;
...
void EvaluationSession::init_namespaces(SourceFile* source) {
  ...
  for (auto nsName : source->getNamespaceNamesOrdered()) {
    ...
    ContextHandle<UserNamespaceContext> nsContext{Context::create<UserNamespaceContext>(getBuiltinContext(), nsScope, nsName, source)};
