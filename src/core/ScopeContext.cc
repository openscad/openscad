...
boost::optional<const Value&> ScopeContext::lookup_local_variable(const std::string& name) const
{
  ...
  // TODO: coryrc - special variables are preserved, so you *could* look them up in usings... but should we? currently returns undef but we can save time by skipping all this if special var.
