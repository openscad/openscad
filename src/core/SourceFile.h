...
public:
  ...
  SourceFile(std::string path, std::string filename, std::shared_ptr<LocalScope> new_scope);
   ...
   */
  const std::vector<std::string>& getNamespaceNamesOrdered() const { return this->namespaceNamesOrdered; }
...
private:
  ...
  static const std::vector<std::string> emptyStringVector; // Just exists so getNamespaceUsedLibrariesReverseOrdered can be const and return reference.
