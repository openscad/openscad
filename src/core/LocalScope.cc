...
// For example, the first time this is called, context is a fresh FileContext with parent context of a BuiltinContext; context's scope is the root_file's scope;
// and the target is a fresh RootNode which is the top-level evaluated output.
...
std::shared_ptr<AbstractNode> LocalNamespaceScope::instantiateModules(const std::shared_ptr<const Context>& context,
						 const std::shared_ptr<AbstractNode>& target) const
{
  throw InvalidInstantiationException("Shouldn't be able to instantiate modules in a namespace"); return NULL;
}
