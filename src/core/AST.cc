...
std::string Location::toRelativeString(std::string docPath) const {
  if (this->isNone()) return "location unknown";
  return "in file " + boostfs_uncomplete((*path), std::move(docPath)).generic_string() + ", " + "line " + std::to_string(this->firstLine());
}
