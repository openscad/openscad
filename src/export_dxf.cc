...
static void export_dxf_footer(std::ostream &output)
{
	output << "  0\n"
				 << "ENDSEC\n";
...
/*!
	Saves the current Polygon2d as DXF to the given output stream.
 */
static void append_dxf_geometry(const Polygon2d &poly, std::ostream &output)
{
...
static void export_dxf_geometry(const shared_ptr<const Geometry> &geom, std::ostream &output)
{
	if (const auto geomlist = dynamic_pointer_cast<const GeometryList>(geom)) {
		for(const auto &item : geomlist->getChildren()) {
			export_dxf_geometry(item.second, output);
		}
	} else if (dynamic_pointer_cast<const PolySet>(geom)) {
		assert(false && "Unsupported file format");
	}
	else if (const auto poly = dynamic_pointer_cast<const Polygon2d>(geom)) {
		append_dxf_geometry(*poly, output);
	} else {
...
{
	// Ensure radix is . (not ,) in output
	setlocale(LC_NUMERIC, "C");
	// Set back to default locale when leaving the scope
	const std::shared_ptr<int> localeGuard(nullptr, [](int *) { setlocale(LC_NUMERIC, ""); });

	export_dxf_header(output);
	export_dxf_geometry(geom, output);
	export_dxf_footer(output);
}
