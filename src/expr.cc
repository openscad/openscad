...
ValuePtr LcEach::evaluate(const Context *context) const
{
    ...
    if (v->type() == Value::ValueType::RANGE) {
        ...
        if (steps >= 1000000) {
            PRINTB("WARNING: Bad range parameter in for statement: too many elements (%lu), %s", loc.toString(context->documentPath()));
        } else {
...
ValuePtr LcFor::evaluate(const Context *context) const
{
    ...
    if (it_values->type() == Value::ValueType::RANGE) {
        ...
        uint32_t steps = range.numValues();
        if (steps >= 1000000) {
            PRINTB("WARNING: Bad range parameter in for statement: too many elements (%lu), %s", steps % loc.toString(context->documentPath()));
        } else {
            for (RangeType::iterator it = range.begin();it != range.end();it++) {
