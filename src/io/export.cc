...
bool is3D(FileFormat format)
{
  return format == FileFormat::ASCII_STL || format == FileFormat::BINARY_STL ||
         ...
         format == FileFormat::NEF3 || format == FileFormat::POV || format == FileFormat::SCAD;
}
...
std::vector<FileFormat> all2D()
{
  ...
  for (const auto& item : containers().fileFormatToInfo) {
    if (is2D(item.first)) all2DFormats.push_back(item.first);
  }
  return all2DFormats;
}
...
std::vector<FileFormat> all3D()
{
  ...
  for (const auto& item : containers().fileFormatToInfo) {
    if (is3D(item.first)) all3DFormats.push_back(item.first);
  }
  return all3DFormats;
}
...
/**
 * Exports the geometry as an OpenSCAD polyhedron module.
 * This is useful for debug or generating native SCAD representations of meshes.
 */
...
std::string get_current_iso8601_date_time_utc()
{
  ...
  return STR(std::put_time(gmtime(&time), "%Y-%m-%dT%H:%M:%SZ"));
}