...
void output_ps(std::ostream& output, std::vector<sheetS>& sheets, const plotSettingsS& plot_s)
{
  ...
  output << "%%DocumentMedia: " << plot_s.paperformat << " " << plot_s.paperwidth * factor << " "
         << plot_s.paperheight * factor << "\n";
  ...
  for (auto& sheet : sheets) {
    ...
    for (unsigned int i = 0; i < sheet.lines.size(); i++) {
      ...
      output << "newpath\n";
      output << (xofs + sheet.lines[i].p1[0]) * factor << " " << (yofs + sheet.lines[i].p1[1]) * factor
             << " moveto\n";
      output << (xofs + sheet.lines[i].p2[0]) * factor << " " << (yofs + sheet.lines[i].p2[1]) * factor
             << " lineto\n";
      output << "stroke\n";
    }
    for (unsigned int i = 0; i < sheet.label.size(); i++) {
      output << (xofs + sheet.label[i].pt[0]) * factor << " " << (yofs + sheet.label[i].pt[1]) * factor
             << " moveto\n";
      output << "gsave " << sheet.label[i].rot << " rotate ( " << sheet.label[i].text
             << " ) show grestore \n";
    }
    output << "10 10 moveto\n";
