...
void import_shell(PolySetBuilder& builder, StepKernel& sk, StepKernel::Shell *shell)
{
  ...
  for (size_t i = 0; i < shell->faces.size(); i++) {
    ...
    for (size_t j = 0; j < face->faceBounds.size(); j++) {
      ...
      for (size_t k = 0; k < loop->faces.size(); k++) {
        ...
        if (circ != nullptr) {
          ...
          if (axis->dir2 != nullptr) xdir = axis->dir2->pt;
          else xdir = Vector3d(zdir[1], zdir[2], -zdir[0]);
      ...
      StepKernel::CylindricalSurface *cyl_surf =
        dynamic_cast<StepKernel::CylindricalSurface *>(face->surface);
      do {
        ...
        auto cyl = std::make_shared<CylinderSurface>(cyl_surf->axis->point->pt, cyl_surf->axis->dir1->pt,
                                                     cyl_surf->r);
    ...
    if (n == 1) {
      ...
      for (auto v : vertices) verticesf.push_back(Vector3f(v[0], v[1], v[2]));  // TODO better
