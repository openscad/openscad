...
You will need to ensure that the environment is updated before trying to build OpenSCAD, so that these utilities are available on the PATH. You might do this by restarting Windows, logging out and back in again, or using the `refreshenv` command that [Chocolatey](https://chocolatey.org/install) provides.
